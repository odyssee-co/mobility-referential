...
def integrate_network(urbanaccess_network, headways=False,
    ...
    #Patched from original implementation to avoid memory leak
    urbanaccess_network.net_nodes.dropna(subset=["id"], inplace = True)
    urbanaccess_network.net_edges.dropna(subset=["from", "to"], inplace = True)
...
def save_network(urbanaccess_network, dir=config.settings.data_folder):
    """
    PATCHED VERSION
...
def load_network(dir=config.settings.data_folder):
    """
    PATCHED VERSION
